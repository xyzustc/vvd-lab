module  pdu_1cycle(
  ...
  //IO_BUS
  input [7:0] io_addr,//读写外设的共用数据线.此模块中外设只有几个,且几个是只读的,另几个是只写的.
  input [31:0] io_dout,//向外设写数据的中间工具人复用数据线,根据地址不同分别写到 out0: io_dout[4:0],ready: io_dout[0],out :io_dout[31:0] displayed as 8 leds.
  input io_we,//向外设写数据的使能信号
  output [31:0] io_din,//从外设读数据的数据线,只要地址对就可以读

  //Debug_BUS
  output [7:0] m_rf_addr,//输出到led显示数据,从下面三个来自cpu的数据里面选.
  input [31:0] rf_data,
  input [31:0] m_data,
  input [31:0] pc
);
...
assign m_rf_addr = {{3{1'b0}}, in_r}; //外设通过设施5位switch来指定想查看的reg或mem unit,这导致了只有32种选择,故所有reg都可看但mem只有低32个可看
...
//读外设端口
always @* begin
  case (io_addr)//io_addr: cpu输入的8位地址{8'h0c,8'h10,default},用于读取in或valid两个外设输入或全0,通过32bit线io_din输入到cpu.
